...
const MobileSidebar = ({ user }: Props) => {
  ...
  // Komponent wyświetlający liczbę obserwujących
          ...
          {mobileSidebarLinks.map((link) => {
            ...
            return (
              <li key={link.title} className="w-full">
                <Link
                  href={link.href}
                  className={`flex w-full flex-row items-center gap-x-6 tracking-wider text-xl font-bold p-3 ${
                    isSamePath ? "bg-gray-300/50" : "hover:bg-black-200"
                  } transition-all`}
                >
                  <Image
  ...
  // Stopka bocznego panelu z opcją wylogowania
  const Footer = () => {
    return (
